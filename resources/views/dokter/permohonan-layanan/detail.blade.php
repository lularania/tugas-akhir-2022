...
@section('title', 'Dokter | Detail Permohonan')

@section('header')
    <div class="col-sm-12">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('dokter') }}">Home</a></li>
            <li class="breadcrumb-item"><a href="/dokter/permohonan-layanan-dikonfirmasi">Permohonan</a></li>
                ...
                <table class="table">
                    <tr>
                        <th style="border: none">
                            <a href="{{ route('dokter.permohonan-layanan-dikonfirmasi') }}" class="btn btn-dark">Kembali</a>
                            ...
                            @endif
                            {{-- <a href="{{ route('dokter.permohonan-layanan.done', $permohonan->id) }}" class="btn btn-success {{ $permohonan->id_status == '4' ? '' : 'disabled' }}">Selesai</a> --}}
                        </th>
                    </tr>
                </table>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.formPenanganan').on('submit', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: "/dokter/permohonan-layanan/finish",
                    data: {
            ...
            $('.formFeedbackOnline').on('submit', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: "/dokter/permohonan-layanan/online/{{ $permohonan->id_permohonan }}",
                    data: {
                        'id_permohonan': $(this).find('input[name=permohonan_id]').val(),
            ...
            $('.formFeedbackOffline').on('submit', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: "/dokter/permohonan-layanan/offline/{{ $permohonan->id_permohonan }}",
                    data: {
                        'id_permohonan': $(this).find('input[name=permohonan_id]').val(),
                        'feedback': $(this).find('.feedback').val(),
                        'catatan': $(this).find('.catatan').val(),
                    ...
                    error: function(res, err) {
                        ...
                        //     alert(res.responseJSON.errors.feedback);
                        // }
                        // if (res.responseJSON.errors.catatan != "undefined") {
                        //     alert(res.responseJSON.errors.catatan);
                        // }
                    },
                });
            });
            $('.formBatal').on('submit', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: "/dokter/permohonan-layanan/reject",
                    data: {
                        'id_permohonan': $(this).find('input[name=permohonan_id]').val(),
